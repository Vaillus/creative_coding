...
def base():
    ...
    in_losange = lo.Losange(nw, ne, sw, se, True, 0.03)
    T = 50
    t = 0
    imgs = []
    while t < T:
        ...
        sw.render(img, bold=True)
        se.render(img, bold=True)
        nw.render(img, bold=True)
        ne.render(img, bold=True)
        ...
        for i in range(len(md)-1):
            for j in range(len(md)-1):
                test = lo.Losange(mg[i], md[j], md[j+1], mg[i+1], 0.15, has_lines=True, thickness=2)
                test.render(img)
        ...
        if cv2.waitKey(1) == ord('q'):
            ...
            break
    imgs[0].save('projects/creative_coding/maroc/2.gif', save_all=True, append_images=imgs[1:], optimize=False, duration=50, loop=0)
...
>>>>>>> 3703c0fe459527209b094d386066ab71e72e84a8:maroc/blender/maroc_main.py
